...
const CUISINE_MODULES = {
  pizza: {
    label: "Pizza", icon: "🍕", listTitle: "Pizzerie", styleLabel: "Style",
    styles: ["Neapolitan", "New York", "Roman", "Detroit", "Sicilian", "Other"],
    dishPlaceholder: "Dish ordered (e.g. Margherita)",
  ...
  pasta: {
    label: "Pasta", icon: "🍝", listTitle: "Pasta", styleLabel: "Type",
    styles: ["Carbonara", "Amatriciana", "Cacio e Pepe", "Ragù", "Seafood", "Other"],
    dishPlaceholder: "Dish ordered (e.g. Carbonara)",
    weights: { pasta: 0.30, sauce: 0.25, value: 0.20, ambiance: 0.15, drinks: 0.10 },
    criteria: [
      { key: "pasta",    label: "Pasta Quality",      sub: "texture · freshness · format",   weight: "30%" },
      { key: "sauce",    label: "Sauce",              sub: "depth · balance · technique",    weight: "25%" },
  ...
  sushi: {
    label: "Sushi", icon: "🍱", listTitle: "Sushi", styleLabel: "Type",
    styles: ["Omakase", "A la Carte", "Conveyor Belt", "Chirashi", "Other"],
    dishPlaceholder: "Experience (e.g. Omakase tasting)",
    weights: { fish: 0.30, rice: 0.25, value: 0.20, ambiance: 0.15, drinks: 0.10 },
    criteria: [
      { key: "fish",     label: "Fish Quality",       sub: "freshness · sourcing · variety",    weight: "30%" },
      { key: "rice",     label: "Rice",               sub: "seasoning · texture · temperature", weight: "25%" },
      { key: "value",    label: "Value for Money",    sub: "price vs experience",               weight: "20%" },
      { key: "ambiance", label: "Ambiance & Service", sub: "atmosphere · speed · care",         weight: "15%" },
      { key: "drinks",   label: "Drinks",             sub: "sake · wine · selection",           weight: "10%" },
    ],
  },
  panini: {
    label: "Panini", icon: "🥪", listTitle: "Panini", styleLabel: "Bread",
    styles: ["Focaccia", "Schiacciata", "Classic", "Other"],
    dishPlaceholder: "Filling (e.g. Mortadella & Burrata)",
    weights: { bread: 0.30, filling: 0.25, toppings: 0.20, value: 0.15, ambiance: 0.10 },
    criteria: [
      { key: "bread",    label: "Bread",              sub: "texture · crust · freshness",    weight: "30%" },
      { key: "filling",  label: "Filling",            sub: "quality · generosity · balance", weight: "25%" },
      { key: "toppings", label: "Toppings & Sauce",   sub: "freshness · combinations",       weight: "20%" },
  ...
  restaurants: {
    label: "Restaurants", icon: "🍽️", listTitle: "Ristoranti", styleLabel: "Cuisine",
    styles: ["Asian", "Ethnic", "Other"],
    dishPlaceholder: "Dish ordered",
    weights: { food: 0.35, service: 0.20, value: 0.20, ambiance: 0.15, drinks: 0.10 },
    criteria: [
      { key: "food",     label: "Food Quality",       sub: "taste · technique · presentation",  weight: "35%" },
      { key: "service",  label: "Service",            sub: "attentiveness · speed · knowledge", weight: "20%" },
      { key: "value",    label: "Value for Money",    sub: "price vs experience",               weight: "20%" },
      { key: "ambiance", label: "Ambiance",           sub: "atmosphere · comfort · vibe",       weight: "15%" },
      { key: "drinks",   label: "Drinks",             sub: "wine · cocktails · selection",      weight: "10%" },
    ],
  },
};
...
function calcScore(scores, cuisineKey = "pizza") {
  const weights = CUISINE_MODULES[cuisineKey]?.weights || CUISINE_MODULES.pizza.weights;
...
function handlePrint(entriesToPrint, cuisineKey) {
  const mod      = CUISINE_MODULES[cuisineKey] || CUISINE_MODULES.pizza;
  ...
  const rows = entriesToPrint.map((e, idx) => {
    ...
    return `<tr>
      ...
      <td style="padding:9px 10px;border-bottom:1px solid #f0f0f0;font-size:10px;font-weight:600;color:${tier.color};letter-spacing:1px;text-transform:uppercase;white-space:nowrap;">
        ${tier.icon} ${tier.label}
      </td>
      ${cells}
      <td style="padding:9px 10px;border-bottom:1px solid #f0f0f0;font-size:12px;text-align:center;color:${e.wouldReturn === "Yes" ? "#5B8A5B" : e.wouldReturn === "No" ? "#8B4040" : "#888"};">
        ${e.wouldReturn}
      </td>
    </tr>`;
  }).join("");
...
<html><head>
  <meta charset="UTF-8">
  <title>La Guida — ${mod.listTitle}</title>
  ...
  <div style="border-bottom:2px solid #C4622D;padding-bottom:14px;margin-bottom:24px;display:flex;justify-content:space-between;align-items:flex-end;">
    <div>
      ...
      <div style="font-family:'Playfair Display',Georgia,serif;font-size:26px;font-weight:700;margin:0 0 4px;">La Guida — ${mod.listTitle}</div>
      <div style="font-size:12px;color:#aaa;">Exported ${date} · ${entriesToPrint.length} restaurant${entriesToPrint.length !== 1 ? "s" : ""}</div>
    </div>
    <div style="text-align:right;font-size:10px;color:#ccc;line-height:1.8;">
      ${criteria.map(c => `${c.label} <strong>${c.weight}</strong>`).join("<br>")}
    </div>
  </div>
  <table style="width:100%;border-collapse:collapse;font-size:13px;">
    <thead><tr>
      <th style="${thStyle}">#</th>
      <th style="${thLeft}">Restaurant</th>
  ...
  <div style="margin-top:32px;font-size:10px;color:#ccc;text-align:center;border-top:1px solid #eee;padding-top:12px;">
    La Guida · Personal Restaurant Guide
  </div>
...
const freshForm = (cuisineKey = "pizza") => {
  const mod    = CUISINE_MODULES[cuisineKey] || CUISINE_MODULES.pizza;
  const scores = Object.fromEntries(mod.criteria.map(c => [c.key, 7]));
  return {
    name: "", location: "", style: mod.styles[0],
    dateVisited: new Date().toISOString().split("T")[0],
    dish: "", priceRange: "€€", scores,
    notes: "", wouldReturn: "Yes", cuisine: cuisineKey,
...
function CuisineSwitcher({ active, onChange }) {
  return (
    <div style={{ overflowX: "auto", display: "flex", gap: 6, paddingBottom: 2, scrollbarWidth: "none", msOverflowStyle: "none" }}>
      {Object.entries(CUISINE_MODULES).map(([key, mod]) => (
        <button key={key} onClick={() => onChange(key)}
          style={{
            background:  active === key ? "#C4622D" : "#1C1814",
            border:      `1px solid ${active === key ? "#C4622D" : "#2A2520"}`,
            color:       active === key ? "#F0EBE1" : "#4A4440",
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 2000, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#1C1814", borderRadius: "18px 18px 0 0", padding: "24px 24px 36px", width: "100%", maxWidth: 430, border: "1px solid #2A2520", borderBottom: "none" }}>
        ...
        <div style={{ background: "#0D0B09", border: "1px solid #2A2520", borderRadius: 10, padding: "12px 14px", fontSize: 12, color: "#4A4440", wordBreak: "break-all", marginBottom: 12, lineHeight: 1.6, fontFamily: "monospace" }}>
          {url}
        </div>
        <div style={{ fontSize: 11, color: "#2A2520", marginBottom: 18 }}>⚠️ Photos are stored locally and not synced across devices.</div>
        ...
        <button onClick={onClose} style={{ width: "100%", background: "transparent", border: "1px solid #2A2520", color: "#4A4440", borderRadius: 12, padding: "13px", fontSize: 14, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Done</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  async function saveEntry() {
    ...
    setView("list");
    setEditingId(null);
    setForm(freshForm(activeCuisine));
  }
  ...
  function openAdd()        { setForm(freshForm(activeCuisine)); setEditingId(null); setView("add"); }
  ...
  const activeMod      = CUISINE_MODULES[activeCuisine];
  const selected       = entries.find(e => e.id === selectedId);
  const previewScore   = parseFloat(calcScore(form.scores, form.cuisine).toFixed(1));
  const previewTier    = getTier(previewScore);
  const syncDot        = { idle: "#2A2520", syncing: "#C4622D", ok: "#5B8A5B", error: "#8B4040" }[syncStatus];

  const app    = { fontFamily: "'DM Sans', sans-serif", background: "#0D0B09", minHeight: "100vh", color: "#F0EBE1", maxWidth: 430, margin: "0 auto", position: "relative" };
  const secLbl = { fontSize: 10, letterSpacing: 3, color: "#C4622D", textTransform: "uppercase", marginBottom: 14, fontWeight: 600 };
  const div    = { borderBottom: "1px solid #1C1814" };
  ...
  if (view === "list") {
    ...
    return (
      <div style={{ ...app, paddingBottom: 90 }} className="pg-fade-in">
        <div style={{ padding: "36px 24px 16px", ...div }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
            <div>
              <div style={{ fontSize: 10, letterSpacing: 4, color: "#C4622D", textTransform: "uppercase", marginBottom: 6, fontWeight: 600 }}>Personal Guide · {activeMod.label}</div>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 36, fontWeight: 700, lineHeight: 1, letterSpacing: -1 }}>La Guida</div>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 14, fontStyle: "italic", color: "#3A3530", marginTop: 4 }}>{activeMod.listTitle}</div>
            </div>
            <div style={{ textAlign: "right" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 5, marginTop: 6 }}>
                ...
                <span style={{ fontSize: 10, color: "#2A2520", letterSpacing: 1 }}>{{ idle: "local", syncing: "syncing…", ok: "synced", error: "offline" }[syncStatus]}</span>
              </div>
            </div>
          </div>
          ...
          {cuisineEntries.length > 0 && (
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              <button onClick={() => handlePrint(cuisineEntries, activeCuisine)} style={{ flex: 1, background: "#1C1814", border: "1px solid #2A2520", color: "#7A7470", borderRadius: 9, padding: "9px", fontSize: 12, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              ...
              <button onClick={() => setShowShare(true)} style={{ flex: 1, background: "#1C1814", border: "1px solid #2A2520", color: "#7A7470", borderRadius: 9, padding: "9px", fontSize: 12, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
        ...
        {cuisineEntries.length === 0 ? (
          <div style={{ textAlign: "center", padding: "80px 24px 40px" }}>
            <div style={{ fontSize: 56, marginBottom: 20 }}>{activeMod.icon}</div>
            <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, fontWeight: 600, marginBottom: 10 }}>Start your {activeMod.label} guide</div>
            <div style={{ fontSize: 14, color: "#3A3530", lineHeight: 1.8, fontWeight: 300 }}>Log your first {activeMod.label.toLowerCase()} experience<br />and build your personal guide.</div>
          </div>
        ) : cuisineEntries.map((entry, idx) => {
          const tier = getTier(entry.weightedScore);
          return (
            <div key={entry.id} onClick={() => openDetail(entry.id)} style={{ display: "flex", alignItems: "center", padding: "16px 24px", ...div, cursor: "pointer", gap: 14 }}>
              ...
              {entry.photo
                ...
                : <div style={{ width: 44, height: 44, borderRadius: 8, background: "#1C1814", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>{activeMod.icon}</div>
              }
  ...
  // ╚══════════════════════════════════════════════╝
  if (view === "map") {
    const located   = cuisineEntries.filter(e => e.lat && e.lng);
    const unlocated = cuisineEntries.filter(e => !e.lat);
    ...
    return (
      <div style={{ ...app, height: "100vh", display: "flex", flexDirection: "column" }} className="pg-fade-in">
        <div style={{ padding: "20px 24px 14px", ...div, background: "#0D0B09", zIndex: 10, flexShrink: 0 }}>
          <div style={{ fontSize: 10, letterSpacing: 4, color: "#C4622D", textTransform: "uppercase", marginBottom: 4, fontWeight: 600 }}>Personal Guide · {activeMod.label}</div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12 }}>
            <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, fontWeight: 700 }}>La Mappa</div>
            <div style={{ fontSize: 12, color: "#3A3530" }}>{located.length} of {cuisineEntries.length} located</div>
          </div>
          <CuisineSwitcher active={activeCuisine} onChange={setActiveCuisine} />
        </div>
        ...
        <div style={{ flex: 1, position: "relative", minHeight: 0 }}>
          {cuisineEntries.length === 0 ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", color: "#3A3530" }}>
              <div style={{ fontSize: 48, marginBottom: 16 }}>{activeMod.icon}</div>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, color: "#4A4440" }}>No {activeMod.label} entries yet</div>
            </div>
          ) : (
            <MapContainer center={center} zoom={13} style={{ height: "100%", width: "100%" }} zoomControl={false}>
              ...
              {located.map(entry => (
                <CircleMarker key={entry.id} center={[entry.lat, entry.lng]} radius={13}
                  pathOptions={{ color: "#0D0B09", weight: 2, fillColor: scoreColor(entry.weightedScore), fillOpacity: 0.92 }}
  ...
  if (view === "detail" && selected) {
    ...
    const entryMod  = CUISINE_MODULES[selected.cuisine || "pizza"] || CUISINE_MODULES.pizza;
    return (
      <div style={{ ...app, paddingBottom: 40 }} className="pg-fade-in">
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", ...div, position: "sticky", top: 0, background: "#0D0B09", zIndex: 10 }}>
        ...
        <div style={{ padding: "24px 24px 20px", ...div }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 12, flexWrap: "wrap" }}>
                ...
                <span style={{ fontSize: 12, color: "#2A2520" }}>{entryMod.icon} {entryMod.label}</span>
              </div>
            ...
            <div style={{ textAlign: "right", flexShrink: 0 }}>
              ...
              <div style={{ fontSize: 11, color: "#2A2520" }}>/10</div>
            </div>
          </div>
        ...
        {selected.notes && (
          <div style={{ padding: "24px 24px", ...div }}>
  ...
  // ╚══════════════════════════════════════════════╝
  if (view === "add") {
    const formMod = CUISINE_MODULES[form.cuisine] || CUISINE_MODULES.pizza;
    const canSave = form.name.trim().length > 0 && !saving;
    return (
      <div style={{ ...app, paddingBottom: 60 }} className="pg-fade-in">
        <input ref={fileRef} type="file" accept="image/*" style={{ display: "none" }} onChange={handlePhoto} />

        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", ...div, position: "sticky", top: 0, background: "#0D0B09", zIndex: 10 }}>
          ...
          <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 17 }}>
            {editingId ? "Edit Entry" : `${formMod.icon} ${formMod.label}`}
          </div>
        ...
        <div style={{ padding: "16px 24px 0" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <div style={{ flex: 1, position: "relative" }}>
                <select className="pg-select" value={form.style} onChange={e => setForm(f => ({ ...f, style: e.target.value }))}>
                  {formMod.styles.map(s => <option key={s}>{s}</option>)}
                </select>
            ...
            <input className="pg-input" placeholder={formMod.dishPlaceholder} value={form.dish} onChange={e => setForm(f => ({ ...f, dish: e.target.value }))} />
          </div>
              ...
              <div style={{ display: "flex", gap: 4 }}>
                {PRICE_RANGES.map(p => (
                  <button key={p} onClick={() => setForm(f => ({ ...f, priceRange: p }))}
                    style={{ flex: 1, background: form.priceRange === p ? "#C4622D" : "#1C1814", border: `1px solid ${form.priceRange === p ? "#C4622D" : "#2A2520"}`, color: form.priceRange === p ? "#F0EBE1" : "#4A4440", borderRadius: 9, padding: "9px 0", fontSize: 10, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
                    {p}
                  </button>
                ))}
              </div>
          ...
          {formMod.criteria.map(c => (
            <div key={c.key} style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12 }}>
                <div><span style={{ fontSize: 15, fontWeight: 500 }}>{c.label}</span><span style={{ fontSize: 11, color: "#2A2520", marginLeft: 8 }}>{c.weight}</span></div>
