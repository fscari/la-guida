...
function PrintView({ entries }) {
  const now = new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" });
  return (
    <div className="print-only">
      <div className="print-page">
        <div className="print-header">
          <div className="print-title">La Guida — Pizzerie</div>
          <div className="print-sub">Personal Restaurant Guide</div>
          <div className="print-meta">Exported {now} · {entries.length} restaurant{entries.length !== 1 ? "s" : ""}</div>
        </div>
        ...
        <table className="print-table">
          <thead>
            <tr>
              ...
              <th>Score</th>
              <th>Tier</th>
              {CRITERIA.map(c => <th key={c.key}>{c.label}<br /><span style={{ fontWeight: 400 }}>{c.weight}</span></th>)}
              <th>Return?</th>
            </tr>
          </thead>
          <tbody>
            {entries.map((e, idx) => {
              ...
              return (
                <tr key={e.id}>
                  <td style={{ color: "#bbb", fontWeight: 600, fontSize: 13 }}>{idx + 1}</td>
                  <td>
                    <div className="print-name">{e.name}</div>
                    {e.location && <div className="print-loc">📍 {e.location}</div>}
                    {e.dish     && <div className="print-loc">{e.style} · {e.dish} · {e.priceRange}</div>}
                    {e.notes    && <div className="print-notes">"{e.notes}"</div>}
                  </td>
                  <td>
                    <span className="print-score" style={{ color: scoreColor(e.weightedScore) }}>
                      {e.weightedScore.toFixed(1)}
                    </span>
                    <div style={{ fontSize: 10, color: "#aaa" }}>/10</div>
                  </td>
                  <td>
                    <span className="print-tier" style={{ color: tier.color }}>
                      {tier.icon} {tier.label}
                    </span>
                  </td>
                  {CRITERIA.map(c => (
                    <td key={c.key}>
                      <span className="print-sub-score" style={{ color: scoreColor(e.scores[c.key] || 5), fontWeight: 600 }}>
                        {e.scores[c.key] || "—"}
                      </span>
                    </td>
                  ))}
                  <td style={{ fontSize: 12, color: e.wouldReturn === "Yes" ? "#5B8A5B" : e.wouldReturn === "No" ? "#8B4040" : "#888" }}>
                    {e.wouldReturn}
                  </td>
                </tr>
              );
        ...
        <div className="print-footer">
          La Guida · Personal Pizza Guide · Crafted with care
        </div>
      </div>
    </div>
  );
